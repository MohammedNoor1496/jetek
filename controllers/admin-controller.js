...
const createAdmin = async (req, res) => {
    ...
    const { firstName, lastName, isSubAdmin, isAdmin, password, phone } = req.body;
    const email = req.body.email.toLowerCase();
    let testadmin = await Admin.findOne({ email });
    if (testadmin) return res.status(400).json({ msg: "Admin  Email already exists " });
    let testadmin1 = await Admin.findOne({ phone: req.body.phone });
    if (testadmin1) return res.status(400).json({ msg: "Admin  Phone already exists " })
...
const adminLogin = async (req, res) => {
    ...
    if (!admin) return res.status(400).json({ msg: "Login failed wrong admin credentials" });
    ...
    var token = jwt.sign({ id: admin.id }, process.env.ADMIN_ACCESS_TOKEN_SECRET, {
...
const deleteCaptin = async (req, res) => {
    console.log("deleteUser");
    console.log(req.body);
    Captin.deleteOne({ _id: req.body.id }).then(
        () => {
            res.status(200).json({
                message: 'Deleted!'
            });
    ...
    try {
        const sp = await new Sp(
            ...
            .then(() => {
                ...
                return res.status(201).json({ msg: "Sell Point Catogare  Successfully Registered" });
            })
...
const deleteSpCatogare = async (req, res) => {
    ...
    console.log(req.body);
    Sp.deleteOne({ _id: req.body.id }).then(
        () => {
            res.status(200).json({
                message: 'Deleted!'
            });
...
const addPCatogare = async (req, res) => {
    ...
    if (pcat) return res.status(400).json({ msg: "prouduct catogery ALERADY EXSIST" });
    ...
    try {
        const pcat = await new ProductCategory(
            ...
            .then(() => {
                ...
                return res.status(201).json({ msg: "prouduct catogery Successfully Registered" });
            })
...
const deletePCatogare = async (req, res) => {
    ...
    console.log(req.body);
    ProductCategory.deleteOne({ _id: req.body.id }).then(
        () => {
            res.status(200).json({
                message: 'Deleted!'
            });
...
const updateCaptinBalance = async (req, res) => {
    ...
    try {
        ...
        if (findCaptin) {
            const updateBalance = await Captin.findByIdAndUpdate({ _id: findCaptin.id }, {
                $set: {
